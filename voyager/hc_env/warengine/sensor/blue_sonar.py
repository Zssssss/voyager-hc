...
class Target:
    def __init__(self,sub_lat,sub_lon,sub_alt,v,f=10000,P=1000,target_density=0,sub_name=""):
        self.f = f  # 频率，HZ
        self.v=v
        self.P = P
...
class Environment:
    ...
    def cal_p(self,P,r,h,target_density=0):
        ...
        p = -TL + SL + TS
        #print("example:","p=",P,"SL=",SL,"r=",r,"TL=",TL,"P=",p)
    ...
    def get_passive_info(self,sonar_v,sonar_lat,sonar_lon,sonar_alt,drag_array):
        ...
        for sub in self.subs:
            ...
            if drag_array:#拖曳声呐阵元探测能力更好
                ...
                lat = sub.sub_lat - sonar_lat + random.uniform(-0.0005, 0.0005)
                lon = sub.sub_lon - sonar_lon + random.uniform(-0.0005, 0.0005)
            #print(lat,lon)
    ...
    def emit_and_receive(self,lat,lon,alt, sonar_v,emit_dir,emit_P, emit_f,emit_angle,DT):
        ...
        for sub in self.subs:
            ...
            real_dir=real_dir/np.linalg.norm(real_dir, axis=0)+[random.uniform(-0.01,0.01) for ii in range(0,3)]#加上方位噪声
            ...
            if sub_r>max_r or theta>emit_angle:#不在探测范围内
                print(sub.name,"距离：",sub_r,",方位角：",theta,",不在探测范围内///////////")
                continue
            print(sub.name, "距离：", sub_r, ",方位角：", theta, ",在探测范围内")
            ...
            if rebound_p<=DT:
                continue
            Rebounds.append({"rebound_time":rebound_time,"rebound_f":rebound_f,"rebound_p":rebound_p,"dir":real_dir})
        return Rebounds

class passive_sonar:#被动声呐类
    def __init__(self,sonar_lat,sonar_lon,sonar_alt,v,env,name="声呐",drag_array=False,DT=162.376):
    ...
    def passive_detect(self,active=False,plt_show=True, env=None):#被动声呐探测函数
        '''
        ...
        if env is not None:
            self.env = env
        sonar_receive_p=self.env.get_passive_info(self.v,self.sonar_lat, self.sonar_lon, self.sonar_alt,self.drag_array)
        # print(self.name+"   开始建模被动信号*************************")
        #接收处理声场信号
        signals=[]
        res=[]
        min_f=999999999
        ...
        plt.xlabel("时间/s")
        plt.ylabel("幅值/v")
        plt.title(self.name+'接收声波（时域）', fontsize=9, color='blue')
        plt.subplot(313)
        plt.plot(half_x, normalization_half_y*100,linewidth =1)
        plt.xlabel("频率/Hz")
        plt.ylabel("功率/W")
        plt.title('功率谱', fontsize=9, color='blue')
        plt.show()
...
class Passive_drag_array_sonar:#被动拖曳阵声呐
    def __init__(self,env,rope_len,hydrophone_num,hydrophone_dis):
        ...
        for i in range(0,hydrophone_num):
            ...
            hydrophone = passive_sonar(sonar_lat=12, sonar_lon=120, sonar_alt=0,
                                  v=0, env=env,drag_array=True,DT=30)
    ...
    def solve_eqs(self,e1,e2):
        ...
        return lon,lat
    def analyse_location(self,JT_lat,JT_lon,JT_alt,v_lon,v_lat,theta_rope,theta_hydrophone,thermocline_height=-90, plt_show=False):
        '''
        ...
        for i in range(0,len(self.hydrophones)):
            ...
            # print((lat_i,lon_i,alt_i))

        # 跃变层信息处理
        hydrophones_alt = np.mean(alt_sonar)
        # print('alt_i',alt_sonar)
        env = []
...
def run_env():
    ...
    sub1 = Target(sub_lat=12.02, sub_lon=120, sub_alt=-300, v=-30, f=1000, P=1000, target_density=10,
                  sub_name="潜艇")  # v:靠近声呐方向为-，远离为+
    sub2 = Target(sub_lat=12.015, sub_lon=120, sub_alt=0, v=-5, f=200, P=1000, target_density=5, sub_name="渔船")
    subs = [sub1, sub2]
...
def run_passive_drag_array_sonar(env):
    ...
    theta_rope = 20
    theta_hydrophone = 20
    Drag_array_sonar = Passive_drag_array_sonar(env, rope_len=rope_len, hydrophone_num=35, hydrophone_dis=1)
    res_Drag_array = Drag_array_sonar.analyse_location(JT_lat=12.88, JT_lon=120.01, JT_alt=-20, v_lon=10, v_lat=-30,
                                                       theta_rope=theta_rope, theta_hydrophone=theta_hydrophone, plt_show=False)
    print('12', geod.Inverse(12.88, 120.01, 12.02, 120)['s12'])
...
def test_cal_p():
    env = Environment([])
    env.cal_p(P=100,r=100000,h=0,target_density=0)
    env.cal_p(P=1000, r=100000, h=0, target_density=0)#拖曳声呐，最大距离100,000,DT=30
    env.cal_p(P=1000, r=3000, h=0, target_density=0)#被动声呐，最大距离3000,DT=162.376
    env.cal_p(P=1000, r=7000, h=0, target_density=0)#主动动声呐，最大距离3500,DT=153.896
def main():
    #test_cal_p()
    env=run_env()
    res_Drag_array = run_passive_drag_array_sonar(env)
    return res_Drag_array
if __name__ == '__main__':
    start = time.time()
    res_Drag_array = main()
    print(res_Drag_array)
