...
from ..commands.plane_command import CmdType, RedObjType
from ..commands.sub_command import SubmarineCommand, BlueObjType

# from cmds.PlaneAgent import PlaneDecision
from ..cmds.SubAgent import SubDecision
# from agents.blue_agent import SubDecision
# from agents.red_agent import RedAgent
from ..commands.plane_command import CmdType
from ..commands.sub_command import BlueObjType
from ..entity.plane_control import PlaneControl
from ..obs.blue_obs import BlueGlobalObservation, Jammer, acoustic_bait
from ..obs.env_obs import EnvGlobalObservation
from ..obs.red_obs import RedGlobalObservation, Buoy, passvive_sonar_combination
from geographiclib.geodesic import Geodesic
import time
# from SQ.model.plane_control import PlaneControl
from ..entity.sub_control import SubControl
from ..entity.usv_control import UsvControl
import math
from ..entity.cargo_ship import CargoShipControl
from ..entity.fishing_boat import FishingBoatControl
...
def sub_course(course0, course1):
    course0 %= 360
    course1 %= 360
    delta_angle = abs(course0 - course1)
    if delta_angle > 180:
...
class StartData:
    def __init__(self):
        ...
        self.start = True
class SubInfo:
    def __init__(self, lat=None, lon=None, course=None, vel=None, simtime=None):
        self.lat = lat
        self.lon = lon
        self.course = course
        self.vel = vel
...
class SouthCall:
    def __init__(self, args, task_id, episode_i):
        ...
        self.uav_velocity_list = [[] for _ in range(args.uav_nums)]  # 无人机速度记录
        ...
        self.red_target_recognition_accuracy = 0  # 目标识别准确率
        # 无人艇
        ...
        self.Y9_Call_point = [{} for _ in range(self.args.submarine_nums)]
    ...
    def get_init_pos(self):
        ...
        d = np.random.uniform(15*1.852*1000, 18*1.852*1000)  # 距离修改过 d = np.random.uniform(30*1.852*1000, 32*1.852*1000)
        ...
        g = geod.Direct(lat1=self.args.Huangwei['lat'], lon1=self.args.Huangwei['lon'], azi1=-90,
                        s12=20 * 1.852 * 1000)  # s12=38 * 1.852 * 1000! 修改距离无人艇的距离
        ...
        for id in range(self.args.submarine_nums):
            if id == 0:  # 大鲸级潜艇，海域中心黄尾屿正西50海里，正方形，边长60海里
                ...
                d = np.random.uniform(16 * 1.852 * 1000, 18 * 1.852 * 1000)
        ...
        for id in range(self.args.plane_nums):
            if id == 0:
                ...
                i = 0
                course = np.random.uniform(-130, -150)
                d = np.random.uniform(10 * 1.852 * 1000, 12 * 1.852 * 1000)  # 修改d = np.random.uniform(9 * 1.852 * 1000, 11 * 1.852 * 1000)
        ...
        for id in range(self.args.red_frigate_nums):  # 护卫舰
            if id < 2:
                ...
                course = np.random.uniform(-80, -90)
                d = np.random.uniform(40 * 1.852 * 1000, 50 * 1.852 * 1000)#48 * 1.852 * 1000, 52 * 1.852 * 1000
                g = geod.Direct(lat1=self.args.Diaoyu['lat'], lon1=self.args.Diaoyu['lon'], azi1=course, s12=d)
                self.red_frigate_init_pos[id]['lat'] = g['lat2']
                self.red_frigate_init_pos[id]['lon'] = g['lon2']
    ...
    def init_obs(self):
        ...
        for id in range(red_obs.plane_field.plane_nums):
            ...
            red_obs.plane_field.planes[id].course = geod.Inverse(red_obs.plane_field.planes[id].lat, red_obs.plane_field.planes[id].lon, self.sub_init_pos[0]['lat'],self.sub_init_pos[0]['lon'])['azi1']
        ...
        for id in range(self.args.red_sub_nums):  # 红方潜艇位置初始化
            ...
            if id == 0:
                lon = np.random.uniform(
                    self.args.blue_sub_field['min_lon'] * 2 / 5 + self.args.blue_sub_field['max_lon'] * 3 / 5,
                    self.args.blue_sub_field['min_lon'] / 5 + self.args.blue_sub_field['max_lon'] * 4 / 5)
                lat = self.args.blue_sub_field['max_lat'] + np.random.uniform(0.04, 0.05)
        ...
        print("红方潜艇与蓝方潜艇的距离", geod.Inverse(red_obs.red_sub_field.red_subs[0].lat, red_obs.red_sub_field.red_subs[0].lon, blue_obs.submarines[0].lat, blue_obs.submarines[0].lon)['s12'])
    ...
    def reset(self):
        ...
        self.uav_velocity_list = [[] for _ in range(self.args.uav_nums)]  # 无人机速度记录
    ...
    def setbystate(self, state_dict):
        self.red_obs, self.blue_obs, self.env_obs, obs_message, key_message, self.result_statistics = state_dict["red_obs"], state_dict["blue_obs"], \
                                                                                                             state_dict["env_obs"], state_dict["obs_message"], \
                                                                                                                state_dict["key_message"], state_dict["result_statistics"]
        return state_dict
    ...
    def step(self, command_dict):
        ...
        if len(command_dict['blue_key_message']) > 0:
            key_message.extend(command_dict['blue_key_message'])

        for messsage_ in command_dict["red_message"]:
            if command_dict['red_result']["usv_call_id"] is not None:
                if messsage_["title"] == "无人艇到达应召点":
                    obs_message["red_message"]['usv_message'][command_dict['red_result']["usv_call_id"]].append(messsage_)
            if command_dict['red_result']["uav_call_id"] is not None:
                if messsage_["title"] == "无人机到达应召点":
                    obs_message["red_message"]['uav_message'][command_dict['red_result']["uav_call_id"]].append(messsage_)
                elif messsage_["title"] == "红方发现可疑目标":
                    obs_message["red_message"]['uav_message'][command_dict['red_result']["uav_call_id"]].append(messsage_)

        if len(command_dict['red_key_message']) > 0:
            key_message.extend(command_dict['red_key_message'])
                ...
                if not self.red_obs.call_point[target_id].lat:
                    for id in range(self.red_obs.plane_field.plane_nums):
                        ...
                        if g < 5_000:
                            ...
                            if len(self.Y9_Call_point[target_id]) == 0:
                                self.Y9_Call_point[target_id] = {"lat":g['lat2'], "lon":g['lon2'], 'time':self.blue_obs.simtime}
        ...
        for i in range(self.blue_obs.submarine_nums):
            ...
            if self.blue_obs.submarines[i].vel <= 4:
        ...
        # 记录潜艇信息
        for id in range(len(self.blue_obs.submarines)):
            self.blue_sub_info[id].append(SubInfo(simtime=self.blue_obs.simtime,
                                                  lat=self.blue_obs.submarines[id].lat,
                                                  lon=self.blue_obs.submarines[id].lon,
                                                  course=self.blue_obs.submarines[id].course,
                                                  vel=self.blue_obs.submarines[id].vel))
        ...
        for red_cmd in red_cmds:
            ...
            if type == CmdType.REPORT:
                ...
                if len(self.blue_sub_info[target_id]) < 15 * 60:
                    sub_avg_course = self.blue_sub_info[target_id][-1].course
                else:
                    last_info = self.blue_sub_info[target_id][-15 * 60]
                    cur_info = self.blue_sub_info[target_id][-1]

                    avg_g = geod.Inverse(last_info.lat, last_info.lon, cur_info.lat, cur_info.lon)
                    sub_avg_course = avg_g['azi1']
                cur_info = self.blue_sub_info[target_id][-1]
                cur_course = cur_info.course
                course_error = min(sub_course(sub_avg_course, report_course), sub_course(cur_course, report_course))
                vel_error = abs(report_vel - cur_info.vel)
                report_sub_g = geod.Inverse(cur_info.lat, cur_info.lon, report_lat, report_lon)
                dis_error = report_sub_g['s12']

                flag = False
                # print('dis error: {} course error: {} vel error: {}'.format(dis_error, course_error, vel_error))
                if dis_error <= self.args.error_dis and vel_error <= self.args.error_vel and course_error <= self.args.error_course:
                    flag = True
                    self.red_obs.report_success_time[target_id] += 1
                    # print("红方成功上报蓝方{}号潜艇{}分钟".format(target_id,
                    #                                               self.red_obs.report_success_time[target_id] / 60))
                else:
                    flag = False

                if self.red_obs.report_success_time[target_id] / 60 == 0:
                    message = {"title": "红方成功上报潜艇位置",
                               "detail": f"红方成功上报蓝方潜艇{self.red_obs.report_success_time[target_id] / 60}分钟",
                ...
                if self.args.Usv_params_Sonar:  # 控制拖拽传感器
                    ...
                    if self.red_obs.usv_field.usvs[id].vel <= 20 and g['s12'] < 8000:
                        ...
                        self.red_obs.usv_field.usvs[id].sonar.sonar_control(statu=statu, theta_rope=theta_rope,
                                                                            rope_len=rope_len,
                                                                            theta_hydrophone=theta_hydrophone)
                        if self.red_obs.usv_field.usvs[id].sonar.open_time == 0:
                            message = {"title": f"无人艇打开拖拽声呐传感器",
                                   "detail": f"{id + 1}号无人艇打开拖拽声呐传感器", 'time': self.blue_obs.simtime}
                            obs_message["red_message"]["usv_message"][id].append(message) if message not in \
                                                                                         obs_message["red_message"][
        ...
        for id in range(self.red_obs.uav_field.uav_nums):
            ...
            self.uav_velocity_list[id].append(self.red_obs.uav_field.uavs[id].vel)

            # 记录无人机速度
            # if command_dict['red_result']['uav_speed_record'][id]['start_time']:
            #     # 飞机开始运动
            ...
            #     if command_dict['red_result']['uav_speed_record'][id]['end_time']:
            #         if self.red_obs.simtime == command_dict['red_result']['uav_speed_record'][id]['end_time'][-1]:
            #             describe = command_dict['red_result']['uav_speed_record'][id]['describe'][-1]
        ...
        if self.red_obs.simtime % self.sonar_update_times == 0:
            ...
            if multi_sonar_loc:
                ...
                g = geod.Direct(self.blue_obs.submarines[0].lat, self.blue_obs.submarines[0].lon,s12=random.random()*100, azi1=random.random()*360)
                # self.red_obs.virtual.lat = self.blue_obs.submarines[0].lat
                # self.red_obs.virtual.lon = self.blue_obs.submarines[0].lon
                self.red_obs.virtual.lat = g['lat2']
                self.red_obs.virtual.lon = g['lon2']
                ...
                self.red_obs.virtual.lat, self.red_obs.virtual.lon = None, None
                ...
                if self.red_obs.uav_field.uavs[id].magnetic.touch:
                    if self.red_obs.uav_field.uavs[id].magnetic.touch_time != self.red_obs.simtime or \
                        ...
                        for i, info in enumerate(self.red_obs.uav_field.uavs[id].magnetic.target_pos):
                            detail += f"第{i + 1}个目标的经度为{info['lon']}, 纬度为{info['lat']}, 高度为{info['height']}；"
                        detail = detail[:-1]
        ...
        for id, sub in enumerate(self.blue_obs.submarines):
            if id == 0 and len(self.red_report_infos[id]) > 0:
                last_report_info = self.red_report_infos[id][-1]
                report_infos = list(filter(lambda x: abs(x.simtime - last_report_info.simtime) <= 16 * 60,
                                           self.red_report_infos[id]))
                count = 0
                simtime = report_infos[-1].simtime
                idx = len(report_infos) - 1
                while idx >= 0:
                    e = report_infos[idx].simtime
                    if abs(e - simtime) <= 60:
                        count += 1
                    else:
                        if count < 4:
                ...
                idx = max(idx, 0)
                delta_time = self.red_report_infos[id][-1].simtime - self.red_report_infos[id][idx].simtime
                true_infos = list(filter(lambda x: x.flag, report_infos))
                acc = len(true_infos) / (len(report_infos))
                print('上报准确率: {} 时间: {}'.format(acc, delta_time))
                if delta_time >= 60 * 15:
                    if acc >= self.args.acc:
                        plane_dones[id] = True
                        self.red_target_recognition_accuracy = acc
                        message = {"title": "红方胜利",
                                   "detail": f"红方成功跟踪蓝方{self.red_obs.report_success_time[id] / 60}分钟，准确率为{acc}%",
                                   'time': self.red_obs.simtime}
                        obs_message["red_message"]["uav_message"][0].append(message) if message not in \
                                                                                        obs_message[
                                                                                            "red_message"][
        ...
        if result != 0:
            ...
            self.uav_passive_sonar_survival_rate = uav_passive_sonar_survival / buoy_nums if buoy_nums != 0 else 0  # 被动声呐存活率 
            self.uav_active_sonar_survival_rate = uav_active_sonar_survival / buoy_nums if buoy_nums != 0 else 0 # 主动声呐存活率
            self.uav_total_duration_call_point = command_dict['red_result']['uav_total_duration_call_point']
            self.red_first_identified_sub_time = command_dict['red_result']['red_first_identified_sub_time']
            self.usv_total_duration_call_point = command_dict['red_result']['usv_total_duration_call_point']
            self.uav_call_id = command_dict['red_result']["uav_call_id"]
            self.usv_call_id = command_dict['red_result']["usv_call_id"]
            ...
            self.result_statistics["blue_result"] = {
                "type": self.blue_obs.submarines[sub_id].type,  #
                ...
                "sub_total_navigation_mileage": round(self.blue_obs.submarines[sub_id].mileage, 2),  #
                "sub_total_duration": self.blue_obs.simtime,  #
                "sub_velocity_list": self.sub_velocity_list[sub_id],  #
                "sub_energy_use":round(self.blue_obs.submarines[sub_id].battery.battery_use, 2),
                # "sub_action_list": self.sub_action_list,
                # "sub_state_list": self.sub_state_list,
                "sub_track_times":command_dict["blue_reslut"]["sub_track_times"],
                "large_course_times":command_dict["blue_reslut"]["large_course_times"],
                "sub_low_speed_sailing_duration": self.sub_low_speed_sailing_duration[sub_id],  #
                "sub_high_speed_sailing_duration": self.sub_high_speed_sailing_duration[sub_id],  #
            ...
            self.result_statistics["red_result"] = {
                "call_point": self.Y9_Call_point[0],  #
                "uav_type": self.red_obs.uav_field.uavs[0].type,  #
                "uav_nums": self.red_obs.uav_field.uav_nums,  #
                "uav_id": self.uav_call_id,
                "uav_total_duration_call_point": self.uav_total_duration_call_point[self.uav_call_id],
                "red_first_identified_sub_time": self.red_first_identified_sub_time,
                "uav_total_navigation_mileage": round(self.red_obs.uav_field.uavs[self.uav_call_id].mileage, 2),  #
                "uav_sonar_buoy_num": self.uav_sonar_buoy_num,
                "uav_energy_use":  round(self.red_obs.uav_field.uavs[self.uav_call_id].battery.battery_use, 2),
                ...
                "red_target_recognition_accuracy": round(self.red_target_recognition_accuracy, 4),
                # 'uav_vel_change': self.uav_state_change,
                "usv_type": self.red_obs.usv_field.usvs[0].type,  #
                # "usv_nums": self.red_obs.usv_field.usv_nums,  #
                "usv_id": self.usv_call_id,
                "usv_total_navigation_mileage": round(self.red_obs.usv_field.usvs[self.usv_call_id].mileage, 2),  #
                "usv_energy_use": round(self.red_obs.usv_field.usvs[self.usv_call_id].battery.battery_use, 2),
                "usv_total_duration_call_point": self.usv_total_duration_call_point[self.usv_call_id],
                "uav_velocity_list": self.uav_velocity_list[self.uav_call_id],
...
class EnvTest:
    ...
    def setbystate(self,state_dict):
        return self.setbystate(state_dict=state_dict)
"""
决策生成
"""
...
class Decision:
    ...
    def make_decision(self, raw_obs, task_id=None, episode_i=None):
        ...
        if red_obs.simtime == 0:
            self.plane_agent = RLRedAgent(red_obs)
        ...
        sub_cmds, commands["blue_message"], commands['blue_key_message'], commands['blue_reslut'] = self.sub_agent.make_decision(blue_obs)
        commands["blue_cmds"] = sub_cmds

        if red_obs.simtime >= 0:
            red_cmds = self.plane_agent.make_decision(red_obs, blue_obs, task_id, episode_i)
            commands["red_cmds"], commands["red_message"], commands["red_key_message"], commands['red_result'] = red_cmds
        return commands
